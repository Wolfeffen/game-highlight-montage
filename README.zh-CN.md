...
游戏高光混剪 Skill —— 把多段游戏精彩素材，按专业剪辑方法论做成节奏型混剪视频。全流程由 AI Agent 独立完成：**你交出素材，它交出成片。**

> 本 Skill 为 AI Agent（如 WorkBuddy / Claude Code 类支持 SKILL.md 规范的环境）设计，加载后 Agent 可按其中的工作流独立完成"素材体检 → 选段 → BGM 对齐 → 预切 → 渲染 → 自检交付"全流程。

## 效果展示

**混剪结构** —— 下面预览背后的真实时间线（27 段守望先锋全场最佳 → 46.7s）。刻度线为真实切点，黄金分割收拳点落在 28.9s：

![混剪结构](assets/structure.svg)

**成片预览**（亮相拼贴 → 极限快切 → 黄金分割收拳）：

![效果预览](assets/demo.gif)

## 为什么不是"让 AI 把片段拼起来"

泛泛的提示词只会产出平庸的混剪：击杀均匀分布、音乐不管节奏、特效每次跑都不一样。这个 Skill 编码了三件多数尝试都缺的东西：

1. **经过验证的剪辑方法论** —— 亮相拼贴 → 能量递增 → 鼓点网格上的极限快切 → 完整表现前后呼应 → 官方收尾，最强一击精确落在黄金分割点（总时长 × 0.618）
2. **数据先行，目检兜底** —— 帧差能量定位激战窗口、拉普拉斯方差剔除低清素材，但每个候选段仍要抽帧目检后才能进时间线
3. **可复现** —— 所有效果（zoom punch、切帧闪光、斜切拼版、电影黑边）都是代码。改时间线、重渲染、结果确定性一致

## 能力概览

- **数据驱动选段**：帧差能量（YDIF）定位激战窗口 + 拉普拉斯方差锐度检测剔除低清素材
- **游戏类型适配**：FPS（守望先锋/无畏契约/CS:GO/三角洲）与 MOBA（王者荣耀/英雄联盟）两套节奏档案
- **平台时长参数化**：微博 35s / 抖音 45s / B站 90s / 竖屏 60s 预设配方，黄金分割硬约束公式
...
> 改回 `game-highlight-montage`。从[项目主页](https://wolfeffen.github.io/game-highlight-montage/)
> 下载的 `game-highlight-montage.zip` 解压后已是正确目录名，无需改动。
